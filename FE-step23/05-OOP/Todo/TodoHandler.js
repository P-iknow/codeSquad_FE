...
class TodoHandler {
  ...
  async update(id, statusToChange) {
    try {
      ...
      todos.splice(
        index,
        countSelected,
        this.makeTodo(name, id, tags, statusToChange),
      );
      this.save();
  ...
  async delete(id) {
    try {
      ...
      console.log(
        this.resultMsg.deleteMsg(todos[index].name, todos[index].status),
      );
  ...
  todoRedoExec(undoOrRedo) {
    if (this.history.checkStackCount(this.history[undoOrRedo], 0))
      throw new Error('이전에 undo 를 실행하지 않았습니다.');
    ...
    if (
      (command === 'add' && undoOrRedo === 'undo') ||
      (command === 'delete' && undoOrRedo === 'redo')
    ) {
    ...
    if (
      (command === 'delete' && undoOrRedo === 'undo') ||
      (command === 'add' && undoOrRedo === 'redo')
    ) {
      todos.push(todo);
    }
