...
function checkParam(count, param){
    return isNum(param) && checkParamCount(count, param)
}

function circleArea(...param){
    const r = checkParam(1, param) && param[0];
    return 3.14 * r * r 
}

function recArea(...param) {
    const [w, h] = checkParam(2,param) && [...param]; 
    return w * h;
}

function trepArea(...param) {
    const [t, b, h] = checkParam(3,param) && [...param];
    return (t+b)/2 * h;
}

function pillar(...param) {
    const [r, h] = checkParam(2, param) && [...param];
